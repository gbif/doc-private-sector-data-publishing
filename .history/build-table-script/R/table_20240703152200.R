...
# harvest publishers and datasets from machineTags
list("dataset","organization") %>%
map(~ get_mt("privateSector.gbif.org",type=.x,limit=500) %>%
mutate(type=.x)) %>%
bind_rows() %>%
mutate(link = gsub("https://www.gbif.org/publisher/",uuid))
