...
rgbif::dataset_get(uuid="72e23311-b65a-46d0-bc07-ff0a251b47e1")

if(FALSE) {
dd = ss %>% 
dplyr::filter(pd == "dataset") %>%
select(key,`Activity sector`) %>% 
mutate(name = map_chr(key,~rgbif::dataset_get(uuid=.x)$data$title)) %>%
...
mutate(p_key = map_chr(key,~ rgbif::dataset_get(uuid=.x)$data$publishingOrganizationKey)) %>%
