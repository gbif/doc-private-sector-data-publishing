...
select(key,`Activity sector`) %>% 
mutate(name = map_chr(key,~rgbif::dataset_get(uuid=.x,limit=1)$data$title)) %>%
...
mutate(p_key = map_chr(key,~ rgbif::dataset_get(uuid=.x,limit=1)$data$publishingOrganizationKey)) %>%
